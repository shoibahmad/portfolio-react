 ...
 * Project catalogue.
 *
 * Lifted out of the Projects component so the page can be re-laid-out without
 * touching the content, and so other surfaces (the modal, the resume) can read
 * the same source.
 */
...
export const PROJECTS = [
        ...
        },
        {
            ...
            description: "A scalable web intelligence engine running across 50+ dynamic sources at 10,000+ requests a day with a 98% retrieval success rate. Twenty-four analyzer modules feed 30+ analysis panels, with the Gemini API extracting structured insight from unstructured pages.",
            ...
            solution: "Built a Python data pipeline on FastAPI, Scrapy and Playwright, splitting static fetches from headless rendering so only pages that genuinely need a browser pay for one. Twenty-four analyzer modules run over the retrieved corpus and surface through 30+ panels; the Gemini API turns page text into structured records. Retry, rotation and backoff are what hold the 98% success rate at volume.",
        ...
        },
        {
            ...
            category: "AI/ML",
            date: "Project",
            description: "An offline-capable disease-screening platform for frontline rural health workers, delivered as a client engagement. A local-first sync layer keeps the app fully usable without connectivity, and Gemini Pro drives clinical risk analysis once data reaches the server.",
            challenge: "Frontline health workers operate where connectivity is intermittent or absent, so a server-dependent app is unusable at exactly the moment it is needed. Screening data still has to reconcile without loss or duplication once a device reconnects, and three different roles need different views of it.",
            solution: "Built a Django REST Framework backend behind a React 18 and Vite front end. The offline layer is a local-first store on IndexedDB with a custom SyncQueue handling retry logic, conflict resolution and automatic reconciliation on reconnect. Gemini Pro performs clinical risk analysis; Firebase Auth and Firestore handle identity while PostgreSQL keeps the audit log. Role-based access spans health worker, officer and patient interfaces, with a regional dashboard for high-risk surveillance.",
        ...
        {
            title: "SecureEval AI — Examination Integrity Platform",
            category: "AI/ML",
            date: "Project",
            description: "A dual-interface examination-integrity platform built for Integral University, now live and in active daily institutional use. Real-time multi-face detection and behavioural analysis flag suspicious activity with 90%+ precision, backed by a live admin monitoring dashboard.",
            challenge: "Remote and hall-based examinations had no reliable way to detect impersonation or collusion at scale, and the university had no observable record of what happened during a session. Requirements arrived ambiguous — the definition of \"suspicious\" itself had to be negotiated with stakeholders.",
            solution: "Architected both interfaces end-to-end: a React.js student and invigilator front end on a FastAPI backend, with high-throughput asynchronous REST endpoints and explicit service contracts to hold concurrent student sessions. MediaPipe Face Mesh handles multi-face geometry in the browser; Gemini 1.5 classifies behavioural sequences server-side. Owned solo from design through deployment.",
        ...
        },
        {
            ...
            category: "AI/ML",
            date: "Project",
            description: "A deepfake detection system built on a CNN reaching 92% accuracy on the FaceForensics++ benchmark, served through a tested FastAPI REST service with a live analytics dashboard.",
            ...
            solution: "Trained a convolutional model in PyTorch with OpenCV handling frame extraction and face alignment, evaluated against the FaceForensics++ benchmark rather than a self-selected split. Inference is exposed as a tested FastAPI REST service, with a dashboard showing detection confidence and throughput over time.",
        ...
        {
            title: "Med-AI Vigi — ADR Risk Predictor",
            category: "AI/ML",
            date: "Project",
            description: "An adverse drug reaction risk predictor built on an XGBoost and Random Forest ensemble, reaching an 87% F1-score and cutting false negatives by 18% against the baseline model.",
